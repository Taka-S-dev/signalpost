...
| `N` | 拒否 |
| `A` | 許可 + **この呼び出しを常に許可** |
| `Shift+A` | 許可 + **このツールをこのプロジェクトで常に許可** |
| `Enter` | その セッションの VS Code ウィンドウを前面に出す（`code -r`） |
| `D` | 情報行を消す |
| `P` / `R` / `S` | 色/名前 / ルール一覧 / 設定 |
...
ルールの一致条件はツール名 + 保存時のスコープのみで、ワイルドカードは持たない。
`A`（この呼び出しだけ）が最も狭く、意図より広く許可されることがないようにしてある。
